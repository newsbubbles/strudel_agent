// {"name": "Global Hidden Markov Model", "tags": ["hmm", "generative", "algorithm", "utility", "global"], "tempo": 120, "description": "HMM implementation with shared global hidden state across multiple observation streams", "author": null, "version": "1.0.0", "date": "2025-12-26"}
...
const ghmm = register('ghmm', (id, pat) => pat.withHap(hap => {
  ...
  while (hmmObservedStates[id].length <= p) {
    ...
    const table = hmmTables[id][currentHidden][0][prevObs]
    let nextObs = prevObs
    let acc = 0
